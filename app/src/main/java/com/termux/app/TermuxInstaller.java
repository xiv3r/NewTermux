...
public final class TermuxInstaller {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    // Step 6: Extract bundled zsh plugins + write .zshrc
    ...
    // Source lines written into .zshrc — used as stable markers for enable/disable
    ...
    private static void ensureZshrcAndShell(Context context) {
        ...
        if (zshrc.exists()) {
            // If .zshrc still references OMZ, back it up and regenerate cleanly.
            try {
                String existing = new String(java.nio.file.Files.readAllBytes(zshrc.toPath()), StandardCharsets.UTF_8);
                if (existing.contains("oh-my-zsh")) {
                    java.nio.file.Files.copy(zshrc.toPath(),
                        new File(TermuxConstants.TERMUX_HOME_DIR_PATH, ".zshrc.omz.bak").toPath(),
                        java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                    writeNewZshrc(context, zshrc);
                }
            } catch (Exception e) {
                Logger.logError(LOG_TAG, "Failed to migrate .zshrc: " + e.getMessage());
            }
        } else {
            writeNewZshrc(context, zshrc);
        }

        // Set zsh as default shell via ~/.termux/shell symlink.
        try {
    ...
    private static void writeNewZshrc(Context context, File zshrc) {
        StringBuilder sb = new StringBuilder("# Generated by NewTermux\n");
        ...
        if (NewTermuxSettings.isZshSyntaxHighlightingEnabled(context))
            sb.append("# zsh-syntax-highlighting must be sourced last\n")
              .append(SYNTAX_HL_LINE).append("\n");
    ...
    public static void setZshAutosuggestions(Context context, boolean enable) {
        toggleZshrcSourceLine(context, AUTOSUGGESTIONS_LINE, enable);
    }

    public static void setZshSyntaxHighlighting(Context context, boolean enable) {
        toggleZshrcSourceLine(context, SYNTAX_HL_LINE, enable);
    }

    private static void toggleZshrcSourceLine(Context context, String line, boolean enable) {
        File zshrc = new File(TermuxConstants.TERMUX_HOME_DIR_PATH, ".zshrc");
        if (!zshrc.exists()) {
            ensureZshrcAndShell(context);
            return;
        ...
        try {
            ...
            content = content.replace(line + "\n", "").replace(line, "").stripTrailing();
            if (enable) content = content + "\n" + line + "\n";
